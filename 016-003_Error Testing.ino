...
//#define DEBUG
//#define DEBUG_RC_IN
#define MinSpeed 60            // SETTING MAX AND MIN SPEED LIMITS // <-------needs to change
#define MaxSpeed 120           // SETTING MAX AND MIN SPEED LIMITS //
#define Min_Steering 60        // SETTING MAX AND MIN STEERING LIMITS //
#define Max_Steering 120       // SETTING MAX AND MIN STEERING LIMITS //
#define Min_Throttle 60        // SETTING MAX AND MIN Throttle LIMITS //
#define Max_Throttle 120       // SETTING MAX AND MIN Throttle LIMITS //
...
void loop() {
  ...
  if (mode == mode_PLAYBACK) {  //mode playback
    ...
    if ((dt > data[0]) && (mode != mode_All_STOP)) {
      ...
      Serial.print(String("time = ") + data[0] + String(" : orientationZ = ") + bno.orientationZ);
      ...
      if (CurrentHeading > ThetaPath) headingError = CurrentHeading - headingError;
      if (CurrentHeading < ThetaPath) headingError = CurrentHeading + headingError;
...
void MIXRC() {  //pulse in for RC PWM and filters it to output LHMIX,RHMIX
  ...
  Thrust = (Thrust - BackwardThrustIn) * (Max_Throttle - Min_Throttle);
  ...
  LHMIX = Thrust + (Direction - 90) * (Steering_Sensitivity_dur1 * .05) / 100;  //doing a little DJ Dan and the fresh tunes mixing
  RHMIX = Thrust - (Direction - 90) * (Steering_Sensitivity_dur1 * .05) / 100;
